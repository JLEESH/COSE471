...
def main():
    ...
    if load_model:
        model = Word2Vec("cbow", mode="negative_sampling", learning_rate=learning_rate, load_model=load_model, 
                        model_filename="w2v_model_cbow_ns", pickle_filename="w2v_vars_process_corpus",
                        max_context_dist=5, debug=debug, subsample=subsample)
    else:
        model = Word2Vec("cbow", mode="negative_sampling", learning_rate=learning_rate, load_model=load_model, 
                        model_filename="w2v_model_cbow_ns", pickle_filename="w2v_vars_process_corpus", #to make it explicit that we are not loading a pickle file
                        max_context_dist=5, debug=debug, subsample=subsample)
    ...
    if perform_training:
        ...
        if inf_train:
            model.train(-1, output_filename="w2v_model_cbow_ns",
                        debug=debug, verbose=verbose, train_partial=train_partial)
        else:
            model.train(iterations, output_filename="w2v_model_cbow_ns",
                        debug=debug, verbose=verbose, train_partial=train_partial)
